...
-- 8.6 安全同步 like_count / collect_count（无全表更新警告）
UPDATE experience_post e
SET like_count = (SELECT COUNT(*) FROM experience_post_like l WHERE l.experience_id = e.id)
WHERE e.id > 0;

UPDATE experience_post e
SET collect_count = (SELECT COUNT(*) FROM experience_post_collect c WHERE c.experience_id = e.id)
WHERE e.id > 0;